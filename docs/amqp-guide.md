...
3. Message tab:   set Queue = "my-queue"
4. Click the green plug icon (Connect)
5. Right panel → click "Publish" to send a message
...
The URL bar at the top and four tabs below it:

| Tab | Purpose |
|---|---|
| **Message** | Configure exchange, routing key, queue, and the message body to publish |
...
This is where the **Publish** and **Consume** buttons live, along with the live message log.

**Buttons at the top of the right panel:**

| Button | What it does |
|---|---|
| **Publish** (blue) | Sends the message body to the broker using the configured exchange + routing key |
| **Start Consuming** (green) | Subscribes to the configured queue — incoming messages appear in the log in real-time |
| **Stop Consuming** (red) | Unsubscribes from the queue (replaces "Start Consuming" while active) |
| **Clear** | Clears the message log |

> **Important:** The Publish / Start Consuming buttons are in the **right panel** (the response pane), not in the left request pane. If you don't see them, make sure you're looking at the right side of the screen.
...
| **Queue** | Yes* | The queue to bind/consume from. *Also used as routing key when exchange is empty |
| **Message Body** | Yes | JSON editor for the message payload |

### How to Publish

1. Make sure you are **connected** (green strip visible)
2. Fill in the routing key and/or queue name
3. Write your message body (JSON) in the editor:
   ...
   ```
4. Go to the **right panel** and click the blue **"Publish"** button
5. The message appears in the log as **→ SENT** (blue background)
...
2. Set the **Queue** name in the Message tab (left panel)
3. Go to the **right panel** and click the green **"Start Consuming"** button
...
→ You need to set a **Queue** name in the Message tab first.
...
→ You must be **connected** before publishing. Click the green plug icon first.
